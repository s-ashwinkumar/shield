#!/usr/bin/env bash
# Plain-bash test harness for shield mux (no bats needed).
set -uo pipefail
MUX="$(cd "$(dirname "${BASH_SOURCE[0]}")/../libexec" && pwd)/shield-mux"
PASS=0; FAIL=0
...
# --- Task 1 ---
out="$(SHIELD_MUX_BACKEND=echo "$MUX" _selftest 2>&1)"
assert_eq "echo backend passes args through" "herdr a b c" "$out"
out="$(SHIELD_MUX_BACKEND=bogus "$MUX" _selftest 2>&1)"; rc=$?
assert_contains "unknown backend errors" "unknown SHIELD_MUX_BACKEND" "$out"

# --- Task 2 ---
B() { SHIELD_MUX_BACKEND=echo "$MUX" "$@"; }
assert_eq "tab-new" "herdr tab create --cwd /w/x --label ENG-1 --no-focus" \
  "$(B tab-new --name ENG-1 --cwd /w/x)"
assert_eq "agent-start passes argv after --" \
  "herdr agent start ENG-1 --cwd /w/x --tab t9 --no-focus -- claude --agent coordinator" \
  "$(B agent-start --name ENG-1 --cwd /w/x --tab t9 -- claude --agent coordinator)"
...
assert_eq "tab-find lists tabs" "herdr tab list" "$(B tab-find --name ENG-1)"
assert_eq "space-ensure (find path)" "herdr workspace list" "$(B space-ensure --name myapp --cwd /r)"
assert_eq "space-find" "herdr workspace list" "$(B space-find --name myapp)"
assert_eq "tab-new with workspace" \
  "herdr tab create --workspace wr --cwd /w/x --label ENG-1 --no-focus" \
  "$(B tab-new --name ENG-1 --cwd /w/x --workspace wr)"
...
assert_eq "pane-rename" "herdr pane rename p0 ENG-1" "$(B pane-rename --pane p0 --name ENG-1)"
